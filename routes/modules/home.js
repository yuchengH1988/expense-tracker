...
const tenPercentile = require('../../utils/tenPercentile')
...
router.get('/', async (req, res) => {
  try {
    ...
    //分類過濾包
    const categories = [{ name: '所有類別' }]
    let categoryLable = []
    await Category.find()
      .lean()
      .then((items) => {
        categories.push(...items)
        items.forEach(item => categoryLable.push(item.name))
      })
    ...
    Record.find(filter)
      ...
      .then(records => {
        ...
        res.render('index', { months, records, categories, totalAmount, selectedCategory, selectedMonth, categoryLable })
      })
